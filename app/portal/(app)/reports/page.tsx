...
export default async function PortalReportsPage({ searchParams }: PageProps) {
  ...
  const [dayVisits, prevVisit, nextVisit] = await Promise.all([
  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-6 py-10">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-2">
        <div>
          {prevVisit ? (
            <Link href={`/portal/reports?date=${toYmd(prevVisit.completedAt!)}`} className="app-btn-secondary-sm">
              ← Previous day
            </Link>
          ) : (
        ...
        <div>
          {nextVisit ? (
            <Link href={`/portal/reports?date=${toYmd(nextVisit.completedAt!)}`} className="app-btn-secondary-sm">
              Next day →
            </Link>
          ) : (
