...
/** Shared by /dashboard/compliance (pool service companies) and /cpo/compliance
 * (AquaRunner Compliance, in-house CPOs) -- entirely persona-agnostic per-state
 * regulatory reference content, so the two products render identical logic under their
 * own separate nav/layout. `settingsHref` is the one thing that differs: where to send an
 * admin to set their org's state if it's missing. */
export async function ComplianceView({
  appUser,
  settingsHref,
}: {
  appUser: { organizationId: string; role: string };
  settingsHref: string;
}) {
        ...
        {!hasCommercialPools ? (
          ...
          <div className="text-sm text-brand-muted">
            ...
            <p className="mt-1">
              ...
              {isAdmin ? (
                <>
                  <Link href={settingsHref} className="app-link">
                    Set it in Settings
                  </Link>
                  .
                </>
