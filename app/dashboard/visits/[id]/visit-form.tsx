...
export function VisitForm({ visitId, visitStatus, hasVolume: initialHasVolume, readingFields, chemicalProducts, checklistItems: initialChecklistItems, initialIssues, initialReading, initialPhotoCount, initialPhotos = [], initialDoses, initialStartedAt, initialDosing }: Props) {
  ...
  async function completeVisit() {
    ...
    try {
      ...
      // "TypeError: Load failed"): the POST had already succeeded server-side by the time
      // the connection dropped, so the visit was actually completed even though this
      // technician saw nothing happen and had no way to tell. Safe to just retry either
      // way -- /api/visits/[id]/complete is idempotent once a visit is already COMPLETED.
      setSaveState("error");
  ...
  return (
    <section className="mt-6 space-y-4">
      ...
      {checklistItems.length > 0 ? (
        <div data-tour="visit-checklist" className="app-card">
